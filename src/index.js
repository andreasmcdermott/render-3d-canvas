...
class Entity {
	constructor(mesh, pos = new Vec3(), rot = new Vec3(), scale = new Vec3(1, 1, 1)) {
		this.mesh = mesh;
		this.pos = pos;
		this.rot = rot;
		this.scale = scale;
...
let boxmesh = new Mesh(
	// Front
	new Polygon(new Vec3(-2, 2, -2), new Vec3(2, 2, -2), new Vec3(2, -2, -2), new Vec3(-2, -2, -2)),
	// Back
	new Polygon(new Vec3(2, 2, 2), new Vec3(-2, 2, 2), new Vec3(-2, -2, 2), new Vec3(2, -2, 2)),
	// Left
	new Polygon(new Vec3(-2, 2, 2), new Vec3(-2, 2, -2), new Vec3(-2, -2, -2), new Vec3(-2, -2, 2)),
...
function drawPolygon(e, polygon, cam) {
	...
	for (let i = 0; i <= polygon.vertices.length; ++i) {
		let vec = polygon.vertices[i % polygon.vertices.length].copy().rotate(e.rot).add(e.pos).scale(e.scale).sub(cam.pos);
...
for (let x = 0; x < 20; ++x) {
	for (let z = 0; z < 20; ++z) {
		floormesh.polygons.push(new Polygon(new Vec3(x, 0, z), new Vec3(x + 1, 0, z), new Vec3(x + 1, 0, z + 1), new Vec3(x, 0, z + 1)));
	}
}
...
box3.color = 'hotpink';
box3.lineWidth = 3;
let floor = new Entity(floormesh, new Vec3(-10, 0, 0), new Vec3(), new Vec3(2, 1, 1));
floor.color = 'white';
